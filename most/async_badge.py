...
class AsyncBadge(object):
    ...
    async def obtain_badge_id(self):
        r = await self.session.get(f"/badge/check",
                                   params={"sn": self.sn})
        r.raise_for_status()
        r = r.json()
        status = r["status"]
    ...
    async def is_registered(self):
        return await self.obtain_badge_id() is not None
    ...
    async def login(self):
        if self.badge_id is None:
            badge_id = await self.obtain_badge_id()
            if badge_id is None:
    ...
    async def upload_audio(self, audio_path,
        ...
        with open(audio_path, 'rb') as f:
            resp = await self.session.post(f"/badge/audio/upload",
                                           params={
                                               "start": start,
                                               "end": end,
                                           },
